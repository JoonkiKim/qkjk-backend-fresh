...
export class BoardsService {
  ...
  create({ createBoardInput }: IBoardsServiceCreate): string {
    // IBoardsServiceCreate는 들어오는 input값에 대한 '타입스크립트'임
    ...
    // 2. DB에 접속 후 거기에 데이터를 저장 => Mysql에 데이터 저장했다고 가정하기
...
// Mysql에서 dto, entity는 몽고디비에서의 스키마 역할을 하는거니까 각각의 파일에서 타입스크립트와 그래프큐엘을 위한 타입을 지정해줘야됨 (서버 -> 디비 방향으로 각각 Mutation과 Query 요청이 가는거임)

// 근데 interface를 통해 각 input값에 대한 타입스크립트를 따로 지정해주는 이유는 브라우저에서 서버로 넘어오는 데이터에 대한 타입을 지정해주기 위함인거임, 이걸 해주는 이유는 데이터타입을 명확히 해서 지뢰를 제거한다는 타입스크립트의 의도를 따름
